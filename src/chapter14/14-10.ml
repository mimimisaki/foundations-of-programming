...
let count_A lst = List.length (List.filter (fun gakusei -> match gakusei with
                    {namae = n; tensuu = t; seiseki = s} -> s = "A") lst)
...
let gakusei_sum lst = List.fold_right (fun first rest_result -> match first with
